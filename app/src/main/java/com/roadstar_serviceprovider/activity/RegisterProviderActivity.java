...
import com.roadstar_serviceprovider.adapter.NewCustomSpinnerAdapter;
import com.roadstar_serviceprovider.adapter.ServiceMediumSpinnerAdapter;
...
public class RegisterProviderActivity extends BaseActivity implements RecyclerItemClickListener<DocumentData> {

    ActivityRegisterProviderBinding mBinding;
    DocumentAdapter documentAdapter = null;
    private String TAG = "RegisterDriverActivity";
    private int itemPosition = -1;
    String mServiceTypeCategory = "";
    String mServiceMediumCategory = "";
    ...
    public static void startActivity(Context context, ArrayList<StatesData> arrayList) {
        Intent homeIntent = new Intent(context, RegisterProviderActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBinding = DataBindingUtil.setContentView(this, R.layout.activity_register_provider);
        initToolbar();
        initView();
        initClickListeners();
...
//        mBinding.swipeRefresh.setRefreshing(false);
    ...
    private void fetchDocument() {
        if (NetworkHandler.isConnected(mThis)) {
            ...
            Call<DocumentResponse> call = apiService.getIndividualDocument();
    ...
    private void fetchServiceType() {
        if (NetworkHandler.isConnected(mThis)) {
            AppUtils.hideKeyboard(mThis);

            ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
            Call<ServiceTypeResponse> call = apiService.getServiceType();
            ...
            call.enqueue(new Callback<ServiceTypeResponse>() {
                ...
                public void onResponse(Call<ServiceTypeResponse> call, Response<ServiceTypeResponse> response) {
                    ...
                    if (response.isSuccessful()) {
                        if (response.body().getCode() == 200) {
                            setServiceTypeSpinnerAdapter(response.body().getData());
    ...
    private void setServiceTypeSpinnerAdapter(ArrayList<BaseData> arrayList) {
        NewCustomSpinnerAdapter adapter = new NewCustomSpinnerAdapter(mThis, R.layout.spinner_dropdown_item, arrayList);

        adapter.setDropDownViewResource(R.layout.support_simple_spinner_dropdown_item);
        mBinding.serviceTypeSpinner.setAdapter(adapter);

        mBinding.serviceTypeSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                mServiceTypeCategory = adapter.getId(position);
                fetchServiceMedium(mServiceTypeCategory);
            }
    ...
    private void fetchServiceMedium(String mServiceTypeCategory) {
        if (NetworkHandler.isConnected(mThis)) {
            AppUtils.hideKeyboard(mThis);
            showHideProgressDialog(true);

            ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
            Call<ServiceMediumResponse> call = apiService.getMediumType(mServiceTypeCategory);

            call.enqueue(new Callback<ServiceMediumResponse>() {
                @Override
                public void onResponse(Call<ServiceMediumResponse> call, Response<ServiceMediumResponse> response) {
                    showHideProgressDialog(false);
                    if (response.isSuccessful()) {
                        if (response.body().getCode() == 200) {
                            setServiceMediumSpinnerAdapter(response.body().getData());
                ...
                @Override
                public void onFailure(Call<ServiceMediumResponse> call, Throwable t) {
                    showHideProgressDialog(false);
                    Lg.e(TAG, t.getMessage());
                    AppUtils.showToast(R.string.server_error);
                }
    ...
    private void setServiceMediumSpinnerAdapter(ArrayList<ServiceMediumData> arrayList) {
        ServiceMediumSpinnerAdapter adapter = new ServiceMediumSpinnerAdapter(mThis, R.layout.spinner_dropdown_item, arrayList);

        adapter.setDropDownViewResource(R.layout.support_simple_spinner_dropdown_item);
        mBinding.serviceMediumSpinner.setAdapter(adapter);

        mBinding.serviceMediumSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                mServiceMediumCategory = adapter.getId(position);
            }
    ...
    private boolean isAllFieldsValid() {
        ...
        if (ValidUtils.isBlank(mBinding.etPractical)) {
            AppUtils.showToast(R.string.error_practical_details);
            return false;
        }

        if (ValidUtils.isBlank(mBinding.etSizeMayTransport)) {
            AppUtils.showToast(R.string.error_size_may_transport);
            return false;
        }

        if (ValidUtils.isBlank(mBinding.etDepartFrom)) {
            AppUtils.showToast(R.string.error_depart);
            return false;
        }

        if (ValidUtils.isBlank(mBinding.etArriveTo)) {
            AppUtils.showToast(R.string.error_arrive);
            return false;
        }
    ...
    private void onButtonClicked() {
        if (NetworkHandler.isConnected(mThis)) {
            ...
            RequestBody journyMedium = RequestBody.create(MediaType.parse("text/plain"), "2");
            RequestBody practicalDetail = RequestBody.create(MediaType.parse("text/plain"), mBinding.etPractical.getText().toString());
            RequestBody sizeOfLoad = RequestBody.create(MediaType.parse("text/plain"), mBinding.etSizeMayTransport.getText().toString());
            RequestBody departureFrom = RequestBody.create(MediaType.parse("text/plain"), mBinding.etDepartFrom.getText().toString());
            RequestBody arrivalTo = RequestBody.create(MediaType.parse("text/plain"), mBinding.etArriveTo.getText().toString());
            ...
            Call<RegisterResponse> call = apiService.registerProvider(
                    firstName,
                    lastName,
                    address,
                    mobile,
