...
import com.roadstar_serviceprovider.databinding.ActivityRegisterDriverBinding;
...
public class RegisterDriverActivity extends BaseActivity implements RecyclerItemClickListener<DocumentData> {

    private static final String TAG = "RegisterProviderActivity";
    ActivityRegisterDriverBinding mBinding;
    DocumentAdapter documentAdapter = null;
    private String mLoadSizeCategory = "";
    private String mLoadSizeName = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBinding = DataBindingUtil.setContentView(this, R.layout.activity_register_driver);
        initToolbar();
        initView();
        initClickListeners();
    ...
    private void fetchDocument() {
        if (NetworkHandler.isConnected(mThis)) {
            ...
            Call<DocumentResponse> call = apiService.getDriverDocument();
    ...
    private void fetchLoadSize() {
        if (NetworkHandler.isConnected(mThis)) {
            AppUtils.hideKeyboard(mThis);

            ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
            Call<LoadSizeResponse> call = apiService.getLoadSize();

            call.enqueue(new Callback<LoadSizeResponse>() {
                @Override
                public void onResponse(Call<LoadSizeResponse> call, Response<LoadSizeResponse> response) {
                    showHideProgressDialog(false);
                    if (response.isSuccessful()) {
                        if (response.body().getCode() == 200) {
                            setLoadSizeSpinnerAdapter(response.body().getData());
                ...
                @Override
                public void onFailure(Call<LoadSizeResponse> call, Throwable t) {
                    showHideProgressDialog(false);
                    Lg.e(TAG, t.getMessage());
                    AppUtils.showToast(R.string.server_error);
                }
    ...
    private void setLoadSizeSpinnerAdapter(ArrayList<BaseData> arrayList) {
        ...
        mBinding.loadSizeSpinner.setAdapter(adapter);

        mBinding.loadSizeSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                mLoadSizeCategory = adapter.getId(position);
                mLoadSizeName = adapter.getName(position);
            }
...
//        mBinding.swipeRefresh.setRefreshing(false);
    ...
    private void onButtonClicked() {
        if (NetworkHandler.isConnected(mThis)) {
            ...
            RequestBody transportId = RequestBody.create(MediaType.parse("text/plain"), "1");
            ...
            RequestBody vehicleId = RequestBody.create(MediaType.parse("text/plain"), "3");

            RequestBody docIds = RequestBody.create(MediaType.parse("text/plain"), ids);

            ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);

            Call<RegisterResponse> call = apiService.registerDriver(
                    firstName,
                    lastName,
                    address,
                    mobile,
